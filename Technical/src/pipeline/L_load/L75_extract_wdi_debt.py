#!/usr/bin/env python3
"""
L75: Extract WDI Debt Detail
External debt stocks, debt service, and composition from WDI.
Stage: L | ID: L75
Project: Gerhard
"""
import os
...
MANIFEST = {
    "id": "L75",
    "name": "Extract WDI Debt Detail",
    "stage": "L",
    "description": "External debt stocks, debt service, and composition from WDI",
    "depends_on": [],
    "inputs": [],
    "outputs": [{"path": "Technical/data/processed/wdi_debt_detail.xlsx"}],
    "timeout": 180,
    "parallel_safe": True,
}
...
INDICATORS = {
    'DT.DOD.DECT.CD': 'external_debt_total_usd',
    'DT.DOD.DECT.GN.ZS': 'external_debt_pct_gni',
    'DT.DOD.DLXF.CD': 'long_term_debt_usd',
    'DT.DOD.DSTC.CD': 'short_term_debt_usd',
    'DT.DOD.DPPG.CD': 'ppg_debt_usd',
    'DT.DOD.DPNG.CD': 'private_nonguaranteed_debt_usd',
    'DT.DOD.DIMF.CD': 'imf_credit_usd',
    'DT.TDS.DECT.CD': 'debt_service_total_usd',
    'DT.TDS.DECT.EX.ZS': 'debt_service_pct_exports',
    'DT.TDS.DECT.GN.ZS': 'debt_service_pct_gni',
    'DT.DOD.DSTC.IR.ZS': 'short_term_pct_reserves',
    'DT.DOD.DSTC.ZS': 'short_term_pct_total',
}

OUTPUT_FILENAME = 'wdi_debt_detail.xlsx'
